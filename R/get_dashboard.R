...
get_dashboard <- function(api_token=get_toggl_api_token(),
  ...
  url <- sprintf("https://toggl.com/reports/api/v2/summary?workspace_id=%s&since=%s&until=%s&user_agent=api_test",
                 get_workspace_id(),
                 format(since,"%Y-%m-%d"),
                 format(until,"%Y-%m-%d")
  ...
  synthese <-    data.frame(id = out$id, out$title, time = out$time) %>%
    select(-color, -hex_color)
  
  
  tache <- out$items %>% 
    setNames(synthese$project) %>% 
    map(~{.x$title <-  .x$title$time_entry;.x})
